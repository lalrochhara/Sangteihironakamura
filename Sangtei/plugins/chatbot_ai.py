...
from Sangtei.core import pyro_cooldown
from Sangtei.helper import check_time_gap, post_to_telegraph
from Sangtei.helper.http import http
from Sangtei.helper.localization import use_chat_lang
from Sangtei.vars import COMMAND_HANDLER, OPENAI_API, SUDO
...
async def openai_chatbot(_, ctx: Message, strings):
    ...
    try:
        ...
        answerlink = await post_to_telegraph(
            False, "Sangtei ChatBot ", html.escape(f"<code>{answer}</code>")
        )
        await msg.edit_msg(
