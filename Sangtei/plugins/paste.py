...
from Sangtei.helper import http, post_to_telegraph, rentry
...
async def wastepaste(_, message):
    ...
    msg = await message.reply_msg("`Pasting to SangteiBin...`")
    data = ""
    limit = 1024 * 1024
    if reply and reply.document:
        if reply.document.file_size > limit:
    ...
    pasted = f"**Successfully pasted your data to SangteiBin<a href='{url}'>.</a>\n\nPaste by {uname}**"
    await msg.edit_msg(pasted, reply_markup=InlineKeyboardMarkup(button))
