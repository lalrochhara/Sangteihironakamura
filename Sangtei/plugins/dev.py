...
from Sangtei import BOT_NAME, app, botStartTime, Sangtei_version, user
from Sangtei.core.decorator import new_task
from Sangtei.helper.eval_helper import format_exception, meval
from Sangtei.helper.functions import extract_user, extract_user_and_reason
from Sangtei.helper.http import http
from Sangtei.helper.human_read import get_readable_file_size, get_readable_time
from Sangtei.helper.localization import use_chat_lang
from Sangtei.vars import AUTO_RESTART, COMMAND_HANDLER, LOG_CHANNEL, SUDO
...
async def log_file(_, ctx: Message, strings):
    ...
    if len(ctx.command) == 1:
        ...
        val = ctx.text.split()
        tail = await shell_exec(f"tail -n {val[1]} -v SangteiLogs.txt")
...
async def update_restart(_, ctx: Message, strings):
    ...
    os.execvp(sys.executable, [sys.executable, "-m", "Sangtei"])
