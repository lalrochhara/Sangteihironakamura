...
from Sangtei import BOT_USERNAME, app, user
from Sangtei.core.decorator.ratelimiter import ratelimiter
from Sangtei.helper import GENRES_EMOJI, http, post_to_telegraph, search_jw
from Sangtei.plugins.dev import shell_exec
from Sangtei.plugins.misc_tools import get_content
from Sangtei.vars import USER_SESSION
...
async def inline_menu(_, inline_query: InlineQuery):
    if inline_query.query.strip().lower().strip() == "":
        ...
        msg = f"""
**[Sangtei✨](https://github.com/NickyLrca):**
...
"""
        ...
        answerss = [
            ...
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    f"<b>Term of Use @{BOT_USERNAME}</b>\n\nhttps://github.com/lalrochhara/Sangtei"
                ),
        ...
        for method in parsemethod:
            if kueri.lower() in method.lower():
                ...
                buttons.row(
                    InlineButton("Give Coffee", url="https://paypal.me/NickyLrca"),
                )
        ...
        for types in parsetypes:
            if kueri.lower() in types.lower():
                ...
                buttons.row(
                    InlineButton("Give Coffee", url="https://NickyLrca"),
                )
                msg = f"<b>{types}</b>\n"
                msg += f"{description}\n\n"
                msg += "<b>Variables:</b>\n"
        ...
        for result in soup.find_all("div", class_="kvH3mc BToiNc UK95Uc"):
            ...
            data.append(
                InlineQueryResultArticle(
                    ...
                    url=link,
                    description=snippet,
                    thumb_url="https://i.ibb.co/mHfjVdB/Bottt.png",
...
async def imdb_inl(_, query):
    ...
    if cbuser == f"{query.from_user.id}":
        try:
            ...
            res_str += "<b>©️ IMDb by</b> @SangteiRoBot"
